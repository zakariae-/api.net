...
namespace WebUtils.Middlewares
{
    ...
    {
        public static IApplicationBuilder UseJsonExceptionMiddleware(this IApplicationBuilder builder, IHostingEnvironment env)
        {
            var jsonExceptionMiddleware = new JsonExceptionMiddleware(env);
            builder.UseExceptionHandler(new ExceptionHandlerOptions { ExceptionHandler = jsonExceptionMiddleware.Invoke });
